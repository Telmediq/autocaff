...
class EventProcessor:
    ...
    def __init__(self):
        ...
        self._avg_topLeft = 0
        self._avg_bottomLeft = 0
        self._avg_topRight = 0
        self._avg_bottomRight = 0

    def mass(self, event):

        self._measureCnt = self._measureCnt + 1

        self._avg_topLeft += event.topLeft
        self._avg_bottomLeft += event.bottomLeft
        self._avg_topRight += event.topRight
        self._avg_bottomRight += event.bottomRight

        print self._avg_topLeft

        print self._measureCnt
        ...
        if (self._measureCnt >= WEIGHT_SAMPLES):
            ...
            self._avg_topLeft = self._avg_topLeft / WEIGHT_SAMPLES
            self._avg_bottomLeft = self._avg_bottomLeft / WEIGHT_SAMPLES
            self._avg_topRight = self._avg_topRight / WEIGHT_SAMPLES
            self._avg_bottomRight = self._avg_bottomRight / WEIGHT_SAMPLES


            weight = {}
            weight["topLeft"] = self._avg_topLeft
            weight["bottomLeft"] = self._avg_bottomLeft
            weight["topRight"] = self._avg_topRight
            weight["bottomRight"] = self._avg_bottomRight
            print "TL: %s TR: %s BL: %s BR: %s" % (weight['topLeft'], weight['topRight'], weight['bottomLeft'], weight['bottomRight'])
            ...
            print raw_json
            requests.put(FIREBASE_URL, data=raw_json)
